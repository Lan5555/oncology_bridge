...
export default function Sidebar({ activePage, sidebarOpen, onNav }: SidebarProps) {
  return (
    <aside className={`sidebar ${sidebarOpen ? "open" : ""}`}>
      <div className="mb-5">
        <div className="mb-2 px-3 text-[10px] font-bold uppercase tracking-wider text-slate-400">Overview</div>
        <button className={`sb-item ${activePage === 'pg-dashboard' ? 'active' : ''}`} onClick={() => onNav('pg-dashboard')}>
          <svg viewBox="0 0 24 24"><rect x="3" y="3" width="72" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/></svg>
          Dashboard
        </button>
      ...
      <div className="mb-5">
        <div className="mb-2 px-3 text-[10px] font-bold uppercase tracking-wider text-slate-400">Pharmacy Tools</div>
        ...
        <button className={`sb-item ${activePage === 'pg-transfers' ? 'active' : ''}`} onClick={() => onNav('pg-transfers')}>
          ...
          Redistribution
          <span className="ml-auto rounded-full px-1.5 py-0.5 text-[9.5px] font-bold text-white bg-red-500 text-white">3</span>
        </button>
      </div>
    
      <div className="mb-5">
        <div className="mb-2 px-3 text-[10px] font-bold uppercase tracking-wider text-slate-400">Clinical</div>
        ...
        <button className={`sb-item ${activePage === 'pg-coldchain' ? 'active' : ''}`} onClick={() => onNav('pg-coldchain')}>
          ...
          Cold Chain Monitor
          <span className="ml-auto rounded-full px-1.5 py-0.5 text-[9.5px] font-bold text-white bg-red-500 text-white animate-pulse">1</span>
        </button>
        <button className={`sb-item ${activePage === 'pg-expiry' ? 'active' : ''}`} onClick={() => onNav('pg-expiry')}>
          <svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><path d="M12 8v4l3 3"/></svg>
          Near-Expiry
          <span className="ml-auto rounded-full px-1.5 py-0.5 text-[9.5px] font-bold text-white bg-amber-500 text-white">7</span>
        </button>
      </div>
    
      {/* Admin section removed — admin console is a top-level route */}
    
      <div className="mb-5 mt-auto pt-4">
